...
async function completeYouth(page: Page) {
  await ask(page, '내가 신청 가능해?');
  await ask(page, '1995년 9월 14일생이고 미혼, 제주 산 지 2년, 통장 24개월에 25회 넣었어요.');
  await ask(page, '월소득 2,669,354원, 총자산 100,000,000원, 부모 자산 200,000,000원이에요.');
  await ask(page, '주택청약종합저축이고 특별공급 당첨 이력 없고 재당첨 제한 없고 해외체류 없고 특례 없고 자녀 없고 태아나 입양도 없어요.');
}
