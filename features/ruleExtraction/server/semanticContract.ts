...
const evidence=list(obj({blockId:str,snippet:str}));
...
export const SEMANTIC_SCHEMA=obj({
  candidateRules:list(obj({candidateRuleId:str,supplyType:enumeration(SUPPLIES),stage:enumeration(['COMMON','PRIORITY','GENERAL','LOTTERY']),category:enumeration(KINDS),ruleKey:str,
    ...
    score:nullableNumber,maxScore:nullableNumber,requiredInputs:list(str),evidence,confidence:enumeration(['HIGH','MEDIUM','LOW']),confidenceReason:str,reviewStatus:enumeration(['REVIEW_REQUIRED'])})),
  unresolvedItems:list(obj({type:enumeration(ISSUE_TYPES),description:str,evidence})),
  conflicts:list(obj({description:str,alternatives:list(obj({value:str,evidence})),resolution:{type:'null'},requiresReview:{type:'boolean',const:true}})),
...
export const SEMANTIC_PROMPT=`assessment-rule-extraction-v2 / pass 2:
...
Unrepresentable composite/exception conditions must be unresolved, not flattened into unconditional eligibility.
