...
export type V3RunnerOptions={plan:'PLAN_A_16'|'PLAN_B_24';retryBudget?:number};
...
export async function runV3BindingBenchmark(document:ParsedDocument,facts:ExtractedFact[],provider:V3BindingProvider,options:V3RunnerOptions){
  const plan=buildCallPlan(document,facts,options.plan),rules:CandidateRule[]=[],responses:SemanticBindingResponse[]=[],runs:V3TaskRun[]=[],warnings:string[]=[];let retryBudget=options.retryBudget??2;
  for(const original of plan.inputs){let input=original,attempt=0,response:SemanticBindingResponse|null=null,lastError:string|null=null,status:TaskStatus='FAILED';
    while(true){try{const raw=await provider.generate(input.task.name,V3_BINDING_PROMPT,providerTaskPayload(input),GEMINI_SAFE_BINDING_SCHEMA);response=validateBindingResponse(raw,input.facts,new Set(input.sources.map(source=>source.sourceId)),new Set(input.task.allowedRoles));status=response.unresolved.length&&!response.bindings.length?'INCOMPLETE':'SUCCESS';break;}catch(error){lastError=code(error);const decision=v3RetryDecision(lastError,attempt);if(retryBudget>0&&decision.retryable){retryBudget--;attempt++;if(decision.action==='SPLIT_ONCE')input=split(input);continue;}status=decision.action==='SKIP_WITH_BACKOFF'?'SKIPPED':lastError==='INCOMPLETE_OUTPUT'?'INCOMPLETE':'FAILED';break;}}
    if(response){responses.push(response);for(const binding of response.bindings){const built=buildCandidateRules(document,input.task,binding,input.facts);rules.push(...built.rules);warnings.push(...built.unresolved.map(item=>`${input.task.name}:${item}`));}warnings.push(...response.unresolved.map(item=>`${input.task.name}:${item.reason}`));}
  ...
  const allBindings=responses.flatMap(response=>response.bindings),exceptionUnresolved=unresolvedExceptions(discoverExceptionSources(document),allBindings);warnings.push(...exceptionUnresolved.map(item=>item.reason));
  return {plan:plan.name,rules,runs,warnings,retryBudgetRemaining:retryBudget,providerCallUpperBound:plan.rows.length+(options.retryBudget??2)};
}
