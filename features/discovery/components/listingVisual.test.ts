...
import {
  ...
  visualAfterError,
} from './listingVisual.ts';
import type { ListingVisual } from './listingVisual.ts';
...
check(() => {
  const visual = resolveListingVisual({ latitude: 37.5, longitude: 127.02 });
...
check(() => {
  const visual = resolveListingVisual({ latitude: null, longitude: null });
  assert.deepEqual(visual, { kind: 'none', reason: 'missing_coordinates' });
});

// NaN 은 좌표가 아니다. 지도 미리보기로 올라가면 안 된다.
check(() => {
  assert.equal(resolveListingVisual({ latitude: Number.NaN, longitude: 127 }).kind, 'none');
  assert.equal(resolveListingVisual({ latitude: 37.5, longitude: Number.NaN }).kind, 'none');
});
...
check(() => {
  ...
  for (const input of inputs) {
    assert.notEqual(resolveListingVisual(input).kind, 'verified_image');
  }
});
...
check(() => {
  ...
  assert.deepEqual(visualAfterError(image), fallback);
});
...
check(() => {
  ...
  assert.deepEqual(visualAfterError(preview), { kind: 'none', reason: 'preview_unavailable' });
});
