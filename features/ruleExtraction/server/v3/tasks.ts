...
import { OFFICIAL_REGION_NAMES } from '../../../discovery/regions.ts';
...
// Keywords describe each task for humans and plans; they are generic terms and statutory values,
// never one announcement's amounts or region. Regions come from the shared registry.
export const V3_TASKS:SemanticTask[]=[
 task('common.regionPriority','COMMON','COMMON',['지역우선','해당지역',...OFFICIAL_REGION_NAMES],['COMMON.REGION.RESIDENCE_MIN','COMMON.REGION.PRIORITY_RATIO']),
 ...
 task('youth.assets','YOUTH','COMMON',['청년','자산','부모','백만원'],['YOUTH.APPLICANT_ASSET_LIMIT','YOUTH.PARENT_ASSET_LIMIT']),
 ...
 task('newlywed.assets','NEWLYWED','COMMON',['신혼부부','자산','백만원'],['NEWLYWED.ASSET_LIMIT']),
 task('newlywed.financial','NEWLYWED','COMMON',['신혼부부','소득','외벌이','맞벌이','자산','백만원'],['NEWLYWED.INCOME.SINGLE_LIMIT','NEWLYWED.INCOME.DUAL_LIMIT','NEWLYWED.ASSET_LIMIT']),
 ...
 task('firstTime.assets','FIRST_TIME','COMMON',['생애최초','자산','백만원'],['FIRST_TIME.ASSET_LIMIT']),
 task('firstTime.financial','FIRST_TIME','COMMON',['생애최초','소득','외벌이','맞벌이','자산','백만원'],['FIRST_TIME.INCOME.SINGLE_LIMIT','FIRST_TIME.INCOME.DUAL_LIMIT','FIRST_TIME.ASSET_LIMIT']),
