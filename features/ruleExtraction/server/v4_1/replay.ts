...
import { validateSemanticBindingV41,type AnnouncementLiteralExpectations,type GuardStatus,type SemanticErrorCategory } from './semanticSafety.ts';
...
export function replayV41(document:ParsedDocument,facts:ExtractedFact[],responses:ReplayResponse[],existingRules:CandidateRule[],highCriticalErrorIds:string[],literalExpectations:AnnouncementLiteralExpectations={}):V41Replay{
  ...
  for(const [taskName,response] of responseByTask){const task=V3_TASKS.find(item=>item.name===taskName);if(!task)continue;const input=buildTaskInput(document,facts,task);for(const binding of response.bindings)guardByBinding.set(`${taskName}:${binding.bindingId}`,validateSemanticBindingV41(document,task,binding,input.facts,facts,literalExpectations));}
