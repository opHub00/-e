...
test('binding contract rejects a fact whose source was not declared by the model',()=>{const t=task('youth.income'),input=buildTaskInput(doc,facts,t),fact=input.facts[0],other=input.sources.find(source=>source.sourceId!==fact.sourceId);assert.ok(other);assert.throws(()=>validateBindingResponse({bindings:[{bindingId:'x',semanticRole:t.allowedRoles[0],factIds:[fact.factId],sourceIds:[other!.sourceId],qualifierSourceIds:[]}],unresolved:[]},input.facts,new Set(input.sources.map(s=>s.sourceId)),new Set(t.allowedRoles)),/FACT_SOURCE_NOT_DECLARED/);});
...
test('PLAN_A_16 attempts core categories and stages for every supply type',()=>{const names=new Set(PLAN_A_16);for(const required of ['youth.basicEligibility','youth.subscription','youth.income','youth.assets','youth.stages','youth.score','newlywed.applicantEligibility','newlywed.subscription','newlywed.financial','newlywed.stages','newlywed.score','firstTime.basicEligibility','firstTime.subscriptionSavings','firstTime.financial','firstTime.taxHistory','firstTime.stages'])assert.ok(names.has(required as any),required);});
test('PLAN_A_16 interleaves supply types before later task rounds',()=>{assert.deepEqual([...PLAN_A_16.slice(0,3)],['youth.basicEligibility','newlywed.applicantEligibility','firstTime.basicEligibility']);assert.deepEqual([...PLAN_A_16.slice(3,6)],['youth.subscription','newlywed.subscription','firstTime.subscriptionSavings']);});
