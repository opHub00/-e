...
const RUN='samdo-ai-v4-plan-a16';
...
const facts=extractFacts(document),plan=buildCallPlan(document,facts,'PLAN_A_16'),scorecard=buildOfflineRetrievalScorecard(document,facts);
if(plan.rows.length!==16||plan.rows.some(row=>row.status!=='READY')||!scorecard.ready)throw new Error('PLAN_A_16_NOT_READY');
await mkdir(dir,{recursive:true});
const write=async(name,value)=>writeFile(resolve(dir,name),JSON.stringify(value,null,2));
await write('samdo-v4-plan-a.json',{schemaVersion:1,mode:'PLAN_A_16',executionOrder:'ROUND_ROBIN_BY_SUPPLY',oracleInput:false,providerCallsAtPlanTime:0,document:{documentId:document.documentId,blocks:document.blocks.length,tables:document.tables.length},factCount:facts.length,retrievalGate:{criticalFacts:scorecard.rows.reduce((sum,row)=>sum+row.expected,0),retrievedFacts:scorecard.rows.reduce((sum,row)=>sum+row.retrieved,0),wrongTopRankedFacts:scorecard.wrongFactCount,missingOperatorFallback:scorecard.missingOperatorFallback,exceptionTasks:scorecard.exceptionTasks,ready:scorecard.ready},tasks:plan.rows});

const configured=providerConfig(process.env);
if(configured.model!=='gemini-2.5-flash')throw new Error('V4_BENCHMARK_MODEL_MUST_BE_GEMINI_2_5_FLASH');
