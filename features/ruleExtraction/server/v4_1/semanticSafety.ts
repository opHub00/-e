...
type LiteralExpectation = { values:number[]; operator?:ExtractedFact['operator']; unit?:string; scope?:ApplicantScope };
const exactExpectations:Record<string,LiteralExpectation> = {
  ...
  'YOUTH.INCOME_LIMIT':{values:[140,5_338_708],operator:'lte',scope:'APPLICANT'},
...
const incomeValues:Record<string,number[]> = {
  'YOUTH.INCOME_LIMIT':[140,5_338_708],
...
function scopeIssues(role:string,facts:ExtractedFact[]):GuardIssue[]{
  const expectation=exactExpectations[role]?.scope;
...
export function validateSemanticBindingV41(document:ParsedDocument,task:SemanticTask,binding:SemanticBinding,taskFacts:ExtractedFact[],allFacts:ExtractedFact[]=taskFacts):SemanticGuardResult{
  ...
  const expected=exactExpectations[binding.semanticRole];
  ...
  const allowedIncome=incomeValues[binding.semanticRole];
