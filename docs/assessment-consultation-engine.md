...
“31살”은 `declaredAgeYears`로 보존하지만 생년월일을 만들지 않는다. 청약의 연령 경계는 날짜에 따라 달라질 수 있으므로 exact birth date가 없으면 엔진은 계속 생년월일을 요청한다. 숫자를 저장하기 전에는 local clause가 진술인지 질문인지 확인한다. “24회 이상이면 몇 점이야?”의 24는 저장하지 않고, “나는 31살인데 39살까지 가능한가요?”에서는 진술 절의 31만 보존한다.
...
Samdo VER1.7은 상단 배지와 세션 시작 안내 한 번으로 상태를 표시한다. 매 답변에는 같은 문구를 반복하지 않는다.

## 상담 안전 보강

- “모르겠어”는 현재 질문을 `deferredFields`에 남기고 다음 영향도 높은 질문으로 이동한다.
- 해외체류 기간만 자연어로 말하고 정확한 날짜가 없으면 날짜를 추정하지 않고 특례 검토 입력으로 보존한다.
- evidence는 현재 intent와 실패·미확인 조건을 우선해 중복을 제거하고 최대 5건만 반환한다.
- 응답에는 `summary`, `reason`, `nextStep`을 선택적으로 제공하며 기존 `message`도 유지한다.
- 공급유형이 바뀌면 전환 문구를 반환하고 이후 결과는 새 공급유형의 deterministic 결과만 사용한다.
