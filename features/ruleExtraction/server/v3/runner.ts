...
import { validateSemanticBindingV41,type AnnouncementLiteralExpectations,type SemanticGuardResult } from '../v4_1/semanticSafety.ts';
...
export type V3RunnerOptions={plan:'PLAN_A_16'|'PLAN_B_24';retryBudget?:number;rateLimitBackoffMs?:number;/** Literal values stated by the announcement being extracted, if known. */literalExpectations?:AnnouncementLiteralExpectations};
...
export async function runV3BindingBenchmark(document:ParsedDocument,facts:ExtractedFact[],provider:V3BindingProvider,options:V3RunnerOptions){
  ...
  for(const original of plan.inputs){let input=original,attempt=0,response:SemanticBindingResponse|null=null,lastError:string|null=null,status:TaskStatus='FAILED';
    ...
    const guards=new Map<string,SemanticGuardResult>();
    while(true){try{const raw=await provider.generate(input.task.name,V3_BINDING_PROMPT,providerTaskPayload(input),GEMINI_SAFE_BINDING_SCHEMA);const parsed=validateBindingResponse(raw,input.facts,new Set(input.sources.map(source=>source.sourceId)),new Set(input.task.allowedRoles)),byId=new Map(input.facts.map(fact=>[fact.factId,fact])),accepted=[],contractUnresolved=[];for(const binding of parsed.bindings){try{assertRoleFactContract(binding.semanticRole,binding.factIds.map(id=>byId.get(id)!).filter(Boolean),input.task);const guard=validateSemanticBindingV41(document,input.task,binding,input.facts,facts,options.literalExpectations);guards.set(binding.bindingId,guard);if(guard.status==='REJECTED')throw new Error(guard.issues.map(item=>item.code).join('|'));accepted.push(binding);}catch(error){contractUnresolved.push({reason:error instanceof Error?error.message:'ROLE_FACT_CONTRACT_VIOLATION',sourceIds:[...new Set(binding.sourceIds)]});}}response={bindings:accepted,unresolved:[...parsed.unresolved,...contractUnresolved]};status=response.unresolved.length&&!response.bindings.length?'INCOMPLETE':'SUCCESS';break;}catch(error){lastError=code(error);const decision=v3RetryDecision(lastError,attempt);if(retryBudget>0&&decision.retryable){retryBudget--;attempt++;if(decision.action==='SPLIT_ONCE')input=split(input);continue;}if(decision.action==='SKIP_WITH_BACKOFF'&&(options.rateLimitBackoffMs??0)>0)await new Promise(resolve=>setTimeout(resolve,options.rateLimitBackoffMs));status=decision.action==='SKIP_WITH_BACKOFF'?'SKIPPED':lastError==='INCOMPLETE_OUTPUT'?'INCOMPLETE':'FAILED';break;}}
    if(response){responses.push({taskName:input.task.name,response});for(const binding of response.bindings){const built=buildCandidateRules(document,input.task,binding,input.facts),guard=guards.get(binding.bindingId);if(guard)for(const rule of built.rules){rule.confidence=guard.confidence.level==='REVIEW_REQUIRED'?'LOW':guard.confidence.level;rule.confidenceReason=guard.confidence.reason;}rules.push(...built.rules);warnings.push(...built.unresolved.map(item=>`${input.task.name}:${item}`));}warnings.push(...response.unresolved.map(item=>`${input.task.name}:${item.reason}`));}
