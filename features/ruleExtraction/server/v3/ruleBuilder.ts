...
import { hasExceptionContext } from './exceptions.ts';
...
const deterministicOperator=(fact:ExtractedFact,equalityRole:boolean)=>{if(fact.operator!==null)return fact.operator;if(equalityRole||fact.type==='BOOLEAN_PHRASE')return 'eq' as const;throw new Error('MISSING_OPERATOR');};
...
export type HostReviewDisposition='AUTO_SAFE_CANDIDATE'|'REVIEW_REQUIRED'|'UNRESOLVED';
export const deriveReviewDisposition=(confidence:HostConfidence,unresolvedCount=0):HostReviewDisposition=>unresolvedCount>0?'UNRESOLVED':confidence.level==='HIGH'?'AUTO_SAFE_CANDIDATE':'REVIEW_REQUIRED';
export function deriveHostConfidence(document:ParsedDocument,binding:SemanticBinding,boundFacts:ExtractedFact[]):HostConfidence {
  ...
  if(binding.qualifierSourceIds.length||hasExceptionContext(texts)||boundFacts.some(f=>f.bindingStatus!=='BOUND'))return {level:'REVIEW_REQUIRED',reason:'예외, 검토 문맥 또는 모호한 fact 결합이 있어 사람 검토가 필요합니다.'};
...
export function buildCandidateRules(document:ParsedDocument,task:SemanticTask,binding:SemanticBinding,facts:ExtractedFact[]):{rules:CandidateRule[];unresolved:string[]} {
  ...
  const confidence=deriveHostConfidence(document,binding,bound),point=bound.find(f=>f.type==='SCORE'&&f.unit==='SCORE_VALUE'),max=bound.find(f=>f.type==='SCORE'&&f.unit==='MAX_SCORE'),isScore=binding.semanticRole.includes('SCORE')&&!binding.semanticRole.includes('SCORELESS');
  if(isScore&&(!point||!max))return {rules:[],unresolved:[`SCORE_POINT_AND_MAX_FACT_REQUIRED:${binding.bindingId}`]};
  const nonScore=bound.filter(f=>f.type!=='SCORE'),thresholdFacts=isScore&&nonScore.length?nonScore:bound.filter(f=>!(f.type==='SCORE'&&f.unit==='MAX_SCORE'));
  const equalityRole=isScore||binding.semanticRole.includes('SUPPLY_RATIO')||binding.semanticRole.includes('PRIORITY_RATIO')||binding.semanticRole.includes('ELIGIBILITY')||binding.semanticRole.includes('APPLICANT_TYPE')||binding.semanticRole.includes('SCORELESS')||binding.semanticRole.includes('EXCEPTION');
  const numeric=new Set(['PERCENT','MONEY','DURATION_MONTHS','DURATION_YEARS','COUNT','AGE']);
  if(thresholdFacts.some(fact=>fact.type==='RANGE'))return {rules:[],unresolved:[`RANGE_REQUIRES_HOST_EXPANSION:${binding.bindingId}`]};
  if(thresholdFacts.some(fact=>numeric.has(fact.type)&&fact.operator===null&&!equalityRole))return {rules:[],unresolved:[`MISSING_OPERATOR:${binding.bindingId}`]};
  ...
  const rules=thresholdFacts.map((fact,index):CandidateRule=>{
    ...
    return {candidateRuleId:`v3:${task.name}:${binding.bindingId}:${index}`,supplyType,stage,category:category(binding.semanticRole),ruleKey:`${binding.semanticRole.toLowerCase()}.${fact.factId}`,
      condition:{input:input(binding.semanticRole,fact),operator:deterministicOperator(fact,equalityRole),value:fact.normalizedValue,values:[]},score:isScore?Number(point?.normalizedValue??fact.normalizedValue):null,maxScore:isScore?Number(max?.normalizedValue??point?.normalizedValue??fact.normalizedValue):null,
