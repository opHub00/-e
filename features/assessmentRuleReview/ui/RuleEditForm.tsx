 ...
 */
export function RuleEditForm({ original, restored, saving = false, onCancel, onSave, onDirtyChange }: Props) {
  ...
  */
  useEffect(() => {
    if (dirty && next) writeReviewDraft({ ruleId: original.ruleKey, edited: next });
  }, [dirty, next, original.ruleKey]);
